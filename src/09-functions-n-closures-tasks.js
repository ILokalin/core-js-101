...
function logger(func, logFunc) {
  return (...args) => {
    const argumentsList = JSON.stringify(args).slice(1, -1);

    logFunc(`${func.name}(${argumentsList}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argumentsList}) ends`);
